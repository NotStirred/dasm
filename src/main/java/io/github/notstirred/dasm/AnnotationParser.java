package io.github.notstirred.dasm;

import io.github.notstirred.dasm.api.provider.ClassProvider;
import io.github.notstirred.dasm.api.redirect.AddFieldToSets;
import io.github.notstirred.dasm.api.redirect.AddMethodToSets;
import io.github.notstirred.dasm.api.redirect.DasmRedirectSet;
import io.github.notstirred.dasm.api.redirect.PartialRedirect;
import io.github.notstirred.dasm.api.transform.DasmRedirect;
import io.github.notstirred.dasm.api.transform.TransformFrom;
import io.github.notstirred.dasm.api.transform.TransformFromClass;
import io.github.notstirred.dasm.transformer.ClassField;
import io.github.notstirred.dasm.transformer.ClassMethod;
import io.github.notstirred.dasm.transformer.redirect.FieldRedirect;
import io.github.notstirred.dasm.transformer.redirect.MethodRedirect;
import io.github.notstirred.dasm.transformer.redirect.RedirectSet;
import io.github.notstirred.dasm.transformer.redirect.TypeRedirect;
import io.github.notstirred.dasm.transformer.target.TargetClass;
import io.github.notstirred.dasm.transformer.target.TargetMethod;
...
public class AnnotationParser {
    ...
    private List<RedirectSet> getRedirectSetsForType(Type setType) {
        return this.redirectSetsByType.computeIfAbsent(setType, t -> {
            ...
            for (InnerClassNode innerClass : classNode.innerClasses) {
                ...
                if (isAnnotationIfPresent(innerClassNode.invisibleAnnotations, io.github.notstirred.dasm.api.redirect.TypeRedirect.class)) {
    ...
    private MethodRedirect parseMethodRedirect(MethodNode methodNode, Type owner, @Nullable Type newOwner, boolean isDstInterface) {
        for (AnnotationNode annotation : methodNode.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(io.github.notstirred.dasm.api.redirect.MethodRedirect.class))) {
                continue;
            }

            Map<String, Object> values = getAnnotationValues(annotation, io.github.notstirred.dasm.api.redirect.MethodRedirect.class);
    ...
    private FieldRedirect parseFieldRedirect(FieldNode fieldNode, Type owner, @Nullable Type newOwner) {
        for (AnnotationNode annotation : fieldNode.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(io.github.notstirred.dasm.api.redirect.FieldRedirect.class))) {
                continue;
            }
    ...
    private TypeRedirect parseTypeRedirect(ClassNode innerClass) {
        for (AnnotationNode annotation : innerClass.invisibleAnnotations) {
            if (!annotation.desc.equals(classToDescriptor(io.github.notstirred.dasm.api.redirect.TypeRedirect.class))) {
                continue;
            }
